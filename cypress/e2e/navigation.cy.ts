describe("navigation spec", () => {
  it("passes", () => {
    ...
    cy.get("#cardNumber").should("be.visible").type("Ahmad Raza Khokhar");
    cy.get("#expiryDate").should("be.visible").type("Chungi Amersadhu, Lahore");
    cy.get("#cvv").should("be.visible").type("+923008039275");
    cy.get("#pay-now").should("be.visible").type("ahmadrazawebexpert@gmail.com");
    cy.end();
  });
});
